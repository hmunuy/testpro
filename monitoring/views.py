...
def home(request):
    ...
    data = Host.objects.raw(SELECT * FROM (SELECT MAX(`insert_time`) as `insert_time`,`id` FROM monitoring_host GROUP BY id ) a LEFT JOIN (SELECT * FROM monitoring_host) b ON a.`insert_time`=b.`insert_time` AND a.`id` = b.`id` GROUP BY b.`id` DESC LIMIT 20)
